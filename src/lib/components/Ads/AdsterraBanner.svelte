<script lang="ts">
    ...
    type AdSize = "300x250" | "320x50" | "728x90" | "160x600" | "320x100";
    ...
    const iframeContent = $derived(`
        ...
        <body>
            ...
            <\/script>
            <script type="text/javascript" src="https://mistletoeframesethel.com/${adKey}/invoke.js"><\/script>
        </body>
        </html>
    `);
    ...
    onMount(() => {
        ...
        if (reloadAfter && reloadAfter > 0) {
            reloadInterval = setInterval(() => {
                cycle++;
                loading = true; // reset loading state on reload
            }, reloadAfter * 1000);
        }

        // Fallback: timer 15s để tắt skeleton nếu iframe không load xong
...
>
    {#key cycle}
        <iframe
            ...
            width={dimensions.width}
            height={dimensions.height}
            style="border: none; overflow: hidden; background: transparent; display: block; position: relative; z-index: 1;"
