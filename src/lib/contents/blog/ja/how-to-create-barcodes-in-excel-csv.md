...
生成したいバーコードの規格（例：Code 128、JAN、Code 39など）を選択する。ツールがCSV内のデータを自動的に解析し、すべてのバーコードを瞬時に生成する。完成した画像は、ZIPファイルとして一括ダウンロードが可能だ。PNG形式は300DPIで出力されるため、ダウンロード後すぐに印刷用データとして使用できる。SVG形式を選択すれば、どんなサイズに拡大・縮小しても画質が劣化しない。
