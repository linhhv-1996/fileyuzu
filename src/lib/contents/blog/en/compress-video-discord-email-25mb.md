---
title: "Compress Video Under 25MB for Discord and Email"
date: "2026-06-23"
description: "Hit the 25MB file limit on Discord and Gmail without guessing. Learn how to compress any video to exactly 24MB on the first try, no re-uploads needed."
...
The real issue with most workarounds is accuracy. Generic compressors make a rough estimate and often return a file that is 25.2MB—still too large—or compress it so aggressively that the video becomes unwatchable. You end up running the process two or three times, which wastes time.

You can avoid all of that by using the [UploadLess Video Compressor](/compress-video), which lets you set an exact target file size so you hit the limit on the first try.
...
This is why you keep ending up with files that are slightly over the limit.
...
To stay safely under the 25MB limit, set your target to **24MB**. This leaves a small buffer for how different platforms measure file size, preventing edge cases where a 25.0MB file still gets rejected.

The result lands at or under your target. No re-running the process, no re-uploading to Google Drive.
...
- **Discord (free account):** 24 MB
- **Gmail / Outlook / Yahoo Mail:** 24 MB
- **WhatsApp:** 15.5 MB (platform limit is 16 MB)
- **Telegram (default):** 49 MB

Setting 1–1.5MB below the stated limit is a reliable practice across all platforms.
...
## Step-by-Step: Compress a Video to Under 25MB
...
3. Type **24** into the target size field.
...
Target File Size mode is specifically useful when you know the ceiling and need to land under it precisely.

---

## Frequently Asked Questions

### Why should I target 24MB instead of exactly 25MB?

Platforms often calculate file size differently than your operating system shows. A file your computer labels as 25.0MB may register as slightly larger on the platform's end. Targeting 24MB gives you a reliable buffer that prevents the file from being rejected at the last second.
...
### Will the video still look acceptable at 24MB if the original is very long?

It depends on the length and content. A 30-second clip compressed to 24MB will look significantly better than a 10-minute video compressed to the same size. For longer videos, consider whether sharing a cloud link is more practical than forcing it into a 24MB limit.
