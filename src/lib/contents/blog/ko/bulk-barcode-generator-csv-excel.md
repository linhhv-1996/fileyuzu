...
일부 바코드 형식은 사용할 수 있는 문자 범위가 제한될 수 있습니다. 내부 관리용이라면 Code 128처럼 비교적 유연한 형식을 사용하는 것이 좋습니다. 형식별 차이와 선택 기준은 [바코드 형식 선택 가이드](/ko/blog/which-barcode-format-to-choose)에서 정리했습니다.
...
PNG도 편하지만, 라벨 인쇄나 크기 조절이 필요한 작업에는 SVG가 더 유리할 수 있습니다. SVG는 크기를 키워도 선명하게 유지되기 때문에 바코드 품질을 유지하기 쉽습니다. 라벨지 배치부터 테스트 출력까지의 전체 과정은 [바코드 라벨 인쇄하는 방법](/ko/blog/how-to-print-barcode-labels)에서 단계별로 설명합니다.
...
하지만 마트, 편의점, 대형 유통망에서 사용하는 정식 상품 바코드는 GS1 Korea(대한상공회의소 유통물류진흥원)를 통해 발급받은 번호여야 합니다. 한국에서 정식 발급된 코드는 880으로 시작하며, 임의로 만든 13자리 숫자와는 다릅니다. 정식 발급이 필요한 경우와 직접 만들어도 되는 경우의 구분은 [상품 바코드 만들기 가이드](/ko/blog/how-to-make-product-barcodes-for-selling)에서 자세히 다룹니다.
...
파일이 준비되면 바코드 생성기를 열고 파일 업로드 모드를 선택합니다. CSV 또는 Excel 파일을 업로드한 뒤 바코드 형식과 라벨 옵션을 확인합니다.
